...
def read_text(file):
    ...
    #src_file = 'D:/NTU ASR/DLHLP/dev/bopomo.trans.txt'
    idx = file.split('/')[-1].split('.')[0]
    with open(src_file, 'r',encoding="utf8") as fp:
        for line in fp:
            #print( idx.split('\\')[-1] , ' ', line.split(' ')[0], '\n' )
            if idx.split('\\')[-1] == line.split(' ')[0]:
    ...
    def __init__(self, path, split, tokenizer, bucket_size):
        ...
        for s in split:
            if s in OFFICIAL_TXT_SRC:
                ...
                with open(join(path, s), 'r',encoding="utf8") as f:
